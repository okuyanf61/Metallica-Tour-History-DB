...
CREATE TABLE IF NOT EXISTS `metallica_db`.`tour` (
  ...
  `set_length` (3,1) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `metallica_db`.`album_to_counter` (
  ...
  `album_id` (11) NOT NULL,
  `count` (11) NOT NULL,
  INDEX `fk_album_to_counter_album1_idx` (`album_id` ASC) VISIBLE,
  INDEX `fk_album_to_counter_tour1_idx` (`event_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `metallica_db`.`event_to_inst` (
  ...
  INDEX `fk_event_to_inst_instrument1_idx` (`instrument_id` ASC) VISIBLE,
  INDEX `fk_event_to_inst_tour1_idx` (`event_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `metallica_db`.`event_url` (
  ...
  PRIMARY KEY (`URL`),
  INDEX `fk_event_url_tour1_idx` (`event_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `metallica_db`.`tour_to_act` (
  `event_id` (11) NOT NULL,
  `act_id` (11) NOT NULL,
  INDEX `fk_tour_to_act_tour1_idx` (`event_id` ASC) VISIBLE,
  INDEX `fk_tour_to_act_acts1_idx` (`act_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `metallica_db`.`tour_to_song` (
  ...
  INDEX `fk_tour_to_song_song1_idx` (`song_id` ASC) VISIBLE,
  INDEX `fk_tour_to_song_tour1_idx` (`event_id` ASC) VISIBLE,
...
-- -----------------------------------------------------
-- Placeholder table for view `metallica_db`.`istanbul_concerts`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `metallica_db`.`istanbul_concerts` (`date` INT, `venue` INT, `festival` INT, `latitude` INT, `longitude` INT, `tour` INT, `set_length` INT, `has_doodle` INT, `has_medley` INT, `event_id` INT, `city_id` INT, `city_name` INT);

-- -----------------------------------------------------
-- procedure CityCountry
-- -----------------------------------------------------
...
-- -----------------------------------------------------
-- View `metallica_db`.`istanbul_concerts`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `metallica_db`.`istanbul_concerts`;
USE `metallica_db`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `metallica_db`.`istanbul_concerts` AS select `metallica_db`.`tour`.`date` AS `date`,`metallica_db`.`tour`.`venue` AS `venue`,`metallica_db`.`tour`.`festival` AS `festival`,`metallica_db`.`tour`.`latitude` AS `latitude`,`metallica_db`.`tour`.`longitude` AS `longitude`,`metallica_db`.`tour`.`tour` AS `tour`,`metallica_db`.`tour`.`set_length` AS `set_length`,`metallica_db`.`tour`.`has_doodle` AS `has_doodle`,`metallica_db`.`tour`.`has_medley` AS `has_medley`,`metallica_db`.`tour`.`event_id` AS `event_id`,`metallica_db`.`city`.`city_id` AS `city_id`,`metallica_db`.`city`.`city_name` AS `city_name` from (`metallica_db`.`tour` join `metallica_db`.`city` on((`metallica_db`.`tour`.`city_id` = `metallica_db`.`city`.`city_id`))) where (`metallica_db`.`city`.`city_name` = 'Istanbul');
